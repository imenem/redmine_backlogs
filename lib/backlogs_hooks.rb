      ...
      def view_issues_show_details_bottom(context={ })
        begin
          ...
          issue_fields_rows do |rows|            
              project = context[:project]
              if issue.is_story?
                rows.left l(:field_story_points), RbStory.find(issue.id).points_display,:class => 'story_points'
                unless issue.remaining_hours.nil?                  
                  rows.right l(:field_remaining_hours), l_hours(issue.remaining_hours),:class => 'remaining_hours'
                end                
                vbe = issue.velocity_based_estimate
                rows.left l(:field_velocity_based_estimate), vbe ? vbe.to_s + ' days' : '-',:class => 'velocity_based_estimate'
                ...
                unless issue.release_id.nil?
                  ...
                  rows.right l(:field_release_relationship), relation_translate, :class => 'release_relationship'
                end
              end

              if issue.is_task? && User.current.allowed_to?(:update_remaining_hours, project) != nil
                rows.left l(:field_remaining_hours), issue.remaining_hours, :class => 'remaining_hours'
              end
          end
        rescue => e
          exception(context, e)
